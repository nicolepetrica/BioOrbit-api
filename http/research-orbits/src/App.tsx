...
function BubbleGraph({ width, height, nodes }: { width: number; height: number; nodes: Node[] }) {
  ...
  return (
    <div className="relative" style={{ width, height }}>
      ...
      {hover && (
        <div
          className="absolute -translate-x-1/2 mt-3 w-80 rounded-2xl border border-white/10 bg-white/10 p-4 text-white backdrop-blur-md shadow-xl"
          style={{
            left: Math.max(160, Math.min(width - 160, hover.x)),
            top: Math.max(16, Math.min(height - 180, hover.y + 40)),
          }}
        >
          <p className="text-sm/5 text-white/80">{hover.node.title || "Category"}</p>
